...
public class GlobalExceptionHandler {
	...
	// Thrown by Spring Security when user is not found in database
	// Occurs inside UserDetailsService.loadUserByUsername(...)
	// Default Spring Security exception
	// Maps to 404 NOT_FOUND (some systems use 401 for security reasons)
	...
	// Example: invalid JWT token, expired token, wrong credentials
	...
	// Occurs when username is correct but password is wrong
	// Occurs during authenticationManager.authenticate(...)
	// Default Spring Security exception
...
//			HttpServletRequest request) {
//
//		return buildErrorResponse(HttpStatus.BAD_GATEWAY, "Product service error: " + ex.getStatusCode(),
//				request.getRequestURI());
//	}
//
